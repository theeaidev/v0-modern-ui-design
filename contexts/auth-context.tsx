...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  console.log(
    "[AUTH CONTEXT] AuthProvider rendering, initialized:",
    authInitialized
  );
  ...
  useEffect(() => {
    console.log(
      "[AUTH CONTEXT] Auth initialization effect running, initialized:",
      authInitialized
    );
    ...
    const initializeAuth = async () => {
      try {
        ...
        if (sessionData?.session) {
          console.log(
            "[AUTH CONTEXT] Session found, setting user and session",
            sessionData.session
          );
          ...
          console.log(
            "[AUTH CONTEXT] No session found, sessionData:",
            sessionData
          );
        }
  ...
  useEffect(() => {
    if (!authInitialized) {
      console.log(
        "[AUTH CONTEXT] Skipping auth state change listener (not initialized)"
      );
      return;
    }
  ...
  const resetPassword = async (email: string) => {
    ...
    try {
      ...
      console.log(
        "[AUTH CONTEXT] Reset password result:",
        !!data,
        "error:",
        !!error
      );
